...
UNUSED static const char libbk__rcsid[] = "$Id: bk_trunc.c,v 1.2 2004/07/08 04:40:18 lindauer Exp $";
...
{
  off_t			pc_size;		///< New size of file
...
main(int argc, char **argv, char **envp)
{
  ...
  const struct poptOption optionsTable[] =
  {
    ...
    {"size", 's', POPT_ARG_STRING, NULL, 's', N_("The new size of the file"), N_("size") },
    POPT_AUTOHELP
    POPT_TABLEEND
  };
  ...
  poptSetOtherOptionHelp(optCon, _("<filename>..."));
  ...
  for (c = 0; c < argc; c++)
  {
    if (truncate(argv[c], pc->pc_size) < 0)
    {
      fprintf(stderr, "%s: Could not truncate %s: %s\n", BK_GENERAL_PROGRAM(B), argv[c], strerror(errno));
      retval++;
    }
  }
