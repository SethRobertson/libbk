...
UNUSED static const char libbk__rcsid[] = "$Id: b_string.c,v 1.130 2008/05/06 23:50:34 jtt Exp $";
	...
	*token = newchar;
	...
	envreplace:
	  ...
	  if (!replace && variabledb)
	  {
	    ...
	    if ((default_val = (strstr(envvar, ":="))) || (default_val = strstr(envvar, ":-")))
	    {
...
bk_memmem(bk_s B, const void *haystack, size_t haystacklen, const void *needle, size_t needlelen)
{
  ...
  BK_RETURN(B, memmem(haystack, haystacklen, needle, needlelen));
#endif /* !HAVE_MEMMEM */
}
