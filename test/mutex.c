...
UNUSED static const char libbk__rcsid[] = "$Id: mutex.c,v 1.5 2004/07/08 04:40:19 lindauer Exp $";
...
static void progrun(bk_s B, struct program_config *pconfig)
{
  ...
  if (pthread_mutex_init(&mutex, NULL) != 0)
    perror("mutex1");
  printf("Ready mutex 2\n");
  if (pthread_mutex_lock(&mutex) != 0)
    perror("mutex1");

  printf("Done mutex\n");

  if (pthread_create(&id, NULL, voidout, &mutex) != 0)
    perror("pthread_create");

  printf("Hello from parent: %d: %p\n",getpid(),&mutex);
  usleep(6000000);
  printf("Hello paret\n");
