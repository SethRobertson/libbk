#if !defined(lint)
static const char libbk__copyright[] = "Copyright © 2009-2010";
...
static void progrun(bk_s B, struct program_config *pc)
{
  ...
  struct bk_bigint bigt;
  float fout;
  u_int testin = 0;
  u_int testlast = 0;
  u_int deltain;
  struct mt_state *mt19937;
  u_int64_t iout;

  if (bk_bigint_init(B, &bigt, 0) < 0)
  {
    fprintf(stderr,"bigint init failed\n");
    exit(2);
  }

  if (!(mt19937 = mt19937_init_genrand64(510)))
  {
    fprintf(stderr,"genrand init failed\n");
    exit(2);
  }

  while (1)
  {
    //    deltain = 65536 * mt19937_genrand64_real1(mt19937);
    deltain = 99999999;
    testin += deltain;
    if (testin < testlast)
      printf("WRAP WRAP WRAP WRAP\n");
    testlast = testin;
    bk_bigint_accumulate(B, &bigt, testin, 0);
    iout = bk_bigint_value(B, &bigt, 0);
    fout = bk_bigint_value(B, &bigt, 0);
    printf("%5d %10u %14lu %14.0f\n",deltain,testin,iout,fout);
  }
