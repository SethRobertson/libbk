 ...
 * @file
 *
 * Test bk_parse_authority functionality
 */
#include <libbk.h>
...
main(int argc, char **argv, char **envp)
{
  ...
  BK_ENTRY_MAIN(B, __FUNCTION__, __FILE__, "test_bua");
  int c;
  int getopterr = 0;
  int debug_level = 0;
...
static int proginit(bk_s B, struct program_config *pc, int argc, char **argv)
{
  BK_ENTRY(B, __FUNCTION__,__FILE__,"test_bua");
...
#define P(n,p) do{ if(p){ printf("%s: **%s**\n", n,p);} } while(0)
...
static void progrun(bk_s B, struct program_config *pc)
{
  ...
  while(fgets(line, sizeof(line), stdin))
  {
    ...
    if (BK_STREQ(line, "end") || BK_STREQ(line, "quit"))
      break;

    if (!(bua = bk_url_parse_authority(B, line, 0)))
    {
      bk_error_printf(B, BK_ERR_ERR, "Could parse: %s\n", line);
      bk_error_repeater_flush(B,0);
      bk_error_strdump(B, NULL, 0, 0);
      continue;
    }
