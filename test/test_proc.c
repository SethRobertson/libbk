#if !defined(lint)
static const char libbk__copyright[] = "Copyright © 2007-2008";
static const char libbk__contact[] = "<projectbaka@baka.org>";
#endif /* not lint */
/*
 * ++Copyright BAKA++
 *
 * Copyright © 2007-2008 The Authors. All rights reserved.
 *
...
main(int argc, char **argv, char **envp)
{
  ...
  BK_ENTRY_MAIN(B, __FUNCTION__, __FILE__, "test_proc");
  int c;
  int getopterr = 0;
  int debug_level = 0;
...
static int proginit(bk_s B, struct program_config *pc)
{
  BK_ENTRY(B, __FUNCTION__,__FILE__,"test_proc");
...
static void progrun(bk_s B, struct program_config *pc)
{
  ...
  if (!(bpil = bk_procinfo_create(B, 0)))
  {
    fprintf(stderr, "Could not create procinfo list\n");
    BK_VRETURN(B);
  }

  printf("%6s %6s CMD\n", "PID", "PPID");

  for (bpi = procinfo_list_minimum(bpil);
       bpi;
       bpi = procinfo_list_successor(bpil, bpi))
  {
    printf("%6d %6d **%s** | **%s** | **%s**\n", bpi->bpi_pid, bpi->bpi_ppid, bpi->bpi_exec_path?bpi->bpi_exec_path:"", bpi->bpi_comm, bpi->bpi_cmdline);
  }

  bk_procinfo_destroy(B, bpil);
  BK_VRETURN(B);
}
