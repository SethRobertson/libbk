 ...
 * @file
 *
 * Test the escaping of an XML comment.
 */
#include <libbk.h>
...
main(int argc, char **argv, char **envp)
{
  ...
  BK_ENTRY_MAIN(B, __FUNCTION__, __FILE__, "test_xml_comment");
  int c;
  int getopterr = 0;
  int debug_level = 0;
...
static int proginit(bk_s B, struct program_config *pc, int argc, char **argv)
{
  BK_ENTRY(B, __FUNCTION__,__FILE__,"test_xml_comment");
...
static void progrun(bk_s B, struct program_config *pc)
{
  ...
  while(1)
  {
    printf("Enter comment line [^D to quit]: ");
    if (!fgets(line, sizeof(line), stdin))
      break;

    if (!bk_string_rip(B, line, NULL, 0))
    {
      bk_error_printf(B, BK_ERR_ERR, "Could not rip input line\n");
      goto error;
    }

    if (bk_string_xml_escape_comment(B, line, &result, 0) < 0)
    {
      bk_error_printf(B, BK_ERR_ERR, "Could not escape comment string\n");
      goto error;
    }
    printf("Result: **%s**\n", result);
  }
