...
main(int argc, char **argv, char **envp)
{
  ...
  BK_ENTRY_MAIN(B, __FUNCTION__, __FILE__, "test_string_expand");
  int c;
  int getopterr = 0;
  int debug_level = 0;
...
static int proginit(bk_s B, struct program_config *pc, int argc, char **argv)
{
  BK_ENTRY(B, __FUNCTION__,__FILE__,"test_string_expand");
...
{
  BK_ENTRY(B, __FUNCTION__,__FILE__,"test_string_expand");
  const char* initial = "${conf_key:=-7.5}";
  ...
  if (!(expanded_string = bk_string_expand(B, (char *)initial, NULL, (const char **)environ, 0)))
  {
    fprintf(stderr,"could not expand string\n");
    exit(1);
  }
